...
function ReadingContent() {
  ...
  const updateBookMutation = useMutation({
    mutationFn: (updatedBook: { id: number; status: string }) =>
                ...
                <button
                  onClick={() =>
                    updateBookMutation.mutate({
                      id: book.id,
                      status: getNextStatus(book.status),
                    })
                  }
                  className="mr-2 px-2 py-1 bg-[#b8bb26] text-[#282828] rounded hover:bg-[#98971a]"
                >
                  Move
                </button>
                <button
  ...
  const getNextStatus = (currentStatus: string) => {
    const statusOrder = ["readingList", "currentlyReading", "finished"];
